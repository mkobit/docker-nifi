...
----
./make_docker.py build \
    --template-substitutions "base_image=java:9-jre,nifi_version=0.6.1,distribution_url=http://apache.mirrors.ionfish.org" \
    --template-file templates/Dockerfile-template \
    --destination-file 0.6.1-openjdk-9.dockerfile \
    --repository mkobit/nifi \
    --tags 0.6.1-openjdk-8,latest
----
